  ...
  if (p.get("format") === "csv" || p.get("format") === "xlsx") {
    ...
    if (p.get("format") === "xlsx") {
      ...
      return new Response(buffer, {
        headers: { "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Content-Disposition": "attachment; filename=bhusatya-records.xlsx" },
      });
    }
    ...
    return new Response([header, ...lines].join("\n"), {
      headers: { "Content-Type": "text/csv", "Content-Disposition": "attachment; filename=bhusatya-records.csv" },
    });
  }

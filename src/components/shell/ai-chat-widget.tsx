...
export function AiChatWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>([{ role: "assistant", text: "Hello! I’m Bhulekh AI. Ask me anything about uploads, OCR, AI/NLP scoring, validation, records, security, or the workflow." }]);
  ...
  return <div className="pointer-events-none fixed inset-0 z-[80]">
    <AnimatePresence>
      ...
      {open && !minimized && <motion.div key="ai-chat-panel" initial={{ opacity: 0, y: 16, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 16, scale: 0.96 }} className="pointer-events-auto absolute bottom-4 right-4 flex h-[min(640px,calc(100dvh-32px))] w-[min(430px,calc(100vw-32px))] flex-col overflow-hidden rounded-2xl border border-[var(--border-interactive)] bg-[var(--bg-secondary)] shadow-2xl shadow-black/70 md:bottom-6 md:right-6">
        ...
        <div className="border-t border-[var(--border-subtle)] p-3"><div className="flex gap-2"><input value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void ask(); }} placeholder="Ask Bhulekh AI…" className={cn(inputCls, "min-w-0 flex-1")} /><Button variant="primary" onClick={() => void ask()} disabled={busy || !question.trim()} aria-label="Send message"><Send size={14} /></Button></div><div className="mt-2 text-[9px] text-muted">Answers are grounded in the Knowledge Bank.</div></div>
      </motion.div>}
    </AnimatePresence>
    {open && minimized && <button onClick={() => setMinimized(false)} className="pointer-events-auto absolute bottom-24 right-4 rounded-full border border-[var(--accent)]/30 bg-[var(--bg-secondary)] px-3 py-1.5 text-xs font-medium text-[var(--accent)] shadow-xl md:right-6">Open Bhulekh AI</button>}
    <button onClick={() => { setOpen((value) => !value); setMinimized(false); }} className="pointer-events-auto absolute bottom-4 right-4 flex size-14 items-center justify-center rounded-full bg-[var(--accent)] text-[#06251c] shadow-xl shadow-[var(--accent-glow)] transition-transform hover:scale-105 md:bottom-6 md:right-6" aria-label="Open Bhulekh AI"><Bot size={24} /></button>
  </div>;
}
...
async function main() {
  ...
  const [admin, officer, reviewer, officer2] = await db
    .insert(users)
    .values([
      { name: "Asha Verma", email: "admin@bhusatya.gov.in", passwordHash: hashPassword("admin123"), role: "admin", color: "#ff9933" },
      { name: "Vikram Singh", email: "officer@bhusatya.gov.in", passwordHash: hashPassword("officer123"), role: "reviewer", color: "#10b981" },
      { name: "Priya Nair", email: "priya@bhusatya.gov.in", passwordHash: hashPassword("officer123"), role: "reviewer", color: "#8b5cf6" },
      { name: "Rohit Mehta", email: "rohit@bhusatya.gov.in", passwordHash: hashPassword("officer123"), role: "officer", color: "#3b82f6" },
    ])
    .returning();
  void officer2;

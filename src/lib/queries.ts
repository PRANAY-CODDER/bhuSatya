...
export type RecordDTO = {
  ...
  stage: string;
  confidence: number;
  extracted: Record<string, { value: unknown; confidence: number; boundingBox?: { x: number; y: number; w: number; h: number } }>;
  ocrText: string | null;
  lat: number | null;
  lng: number | null;
  notes: string | null;
...
export type DashboardStats = {
  ...
  addedToday: number;
  ingestionTrend: { date: string; count: number }[];
...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  const ingestionTrend = Array.from({ length: 7 }, (_, index) => {
    const dayStart = heatStart.getTime() - (6 - index) * 86_400_000;
    return {
      date: new Date(dayStart).toISOString(),
      count: records.filter((r) => r.createdAt.getTime() >= dayStart && r.createdAt.getTime() < dayStart + 86_400_000).length,
    };
  });
  const statusBreakdown = ["verified", "processing", "flagged", "rejected", "pending", "review"].map((status) => ({
    status,
    count: statusCount(status),
  }));
  const districtCounts = new Map<string, number>();
  for (const record of records) districtCounts.set(record.district, (districtCounts.get(record.district) ?? 0) + 1);
